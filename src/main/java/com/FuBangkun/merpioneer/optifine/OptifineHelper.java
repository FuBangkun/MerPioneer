package com.FuBangkun.merpioneer.optifine;
...
public class OptifineHelper {
    ...
    public static void init() {
        ...
        MerPioneer.LOGGER.info("OptiFine detected. Performing highly invasive tweaks to fix water issues.");
        try {
            Class<?> reflectorMethod = Class.forName("net.optifine.reflect.ReflectorMethod");
            Object newReflectorMethod = reflectorMethod.getConstructor(Class.forName("net.optifine.reflect.ReflectorClass"), String.class).newInstance(reflectorMainClass.getDeclaredField("ForgeBiome").get(null), "waterColorMultiplier");
            ...
            MerPioneer.LOGGER.error("An error occured while patching OptiFine", e);
            return;
        }
    ...
    public static void reloadBlockAliases() {
        try {
            ...
            MerPioneer.LOGGER.error("Error reloading OptiFine block aliases", e);
        }
    }
