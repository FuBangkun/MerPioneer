package com.FuBangkun.merpioneer.biome;
...
public abstract class BiomeWaterFogColors {
    ...
    private static void processStringColor(String colorEntry) {
        ...
        if (fields.length != 3) {
            MerPioneer.LOGGER.error("Incorrect syntax for '{}'. Should be modname:biome,color,fogcolor (color and fogcolor may be empty)", colorEntry);
            return;
        }
    ...
    public static int getWaterColorForBiome(Biome biome, int oldColor) {
        ...
        if (oldColor != DEFAULT_WATER_COLOR_112) {
            MerPioneer.LOGGER.info("Potentially missing water color mapping for {}, attempting to fake old appearance", location);
